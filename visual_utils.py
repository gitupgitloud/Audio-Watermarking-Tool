...
def clear_plots(root):
    ...
    # Also destroy any remaining plot widgets
    for widget in root.winfo_children():
        if isinstance(widget, FigureCanvasTkAgg):
            widget.get_tk_widget().destroy()
        elif isinstance(widget, ctk.CTkFrame):  # Clear any plot containers
            widget.destroy()

def display_waveform_in_window(root):
    """Display or refresh the waveform plot"""
    ...
    try:
        current_figure, ax = plt.subplots(figsize=(10, 4))
        ...
        current_canvas = FigureCanvasTkAgg(current_figure, master=root)
        current_canvas.draw()
        current_canvas.get_tk_widget().pack(pady=20, fill='both', expand=True)
        ...
        messagebox.showerror("Error", f"Failed to display waveform: {str(e)}")
...
def display_mel_spectrogram_in_window(root):
    ...
    try:
        ...
        current_figure.colorbar(img, ax=ax, format='%+2.0f dB')
        ...
        messagebox.showerror("Error", f"Failed to display spectrogram: {str(e)}")
...
def display_extracted_watermark(root, watermark, sr, method):
    ...
    try:
        ...
        current_figure, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 5))
        ...
        current_canvas = FigureCanvasTkAgg(current_figure, master=container)
        current_canvas.draw()
        ...
        messagebox.showerror("Error", f"Failed to display extracted watermark: {str(e)}")
